...
install_puppet() {
  ...
  # Ensure /etc/profile.d/puppetlabs.sh exists and add /op92.168.50.2t/puppetlabs/bin/ to PATH globally if not already added
...
set_hostname() {
  # Check if hostname already exists with comp pattern
  current_hostname=$(hostname)
  if [[ "$current_hostname" == comp-* ]]; then
  ...
  mkdir -p $COMPANY_NODES_DIR
  touch $COMPANY_NODES_DIR/${HOSTNAME}
